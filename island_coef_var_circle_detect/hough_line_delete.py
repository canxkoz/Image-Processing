...
HOUGH_SMALL_LINE_PARAMETERS = {
    ...
    'low_threshold': 50,   # Default 50
    'high_threshold': 150, # Default 150
    'rho': 1,  # distance resolution in pixels of the Hough grid                     Default 1
    'theta': np.pi / 2000,  # angular resolution in radians of the Hough grid         Default np.pi / 180
    'threshold': 5,  # minimum number of votes (intersections in Hough grid cell)   Default 15
    'min_line_length': 20,  # minimum number of pixels making up a line              Default 50
    'max_line_gap': 2,  # maximum gap in pixels between connectable line segments   Default 20
}
...
def show_im(image, name='empty'):
    ...
    cv2.moveWindow(name, 300,300)  # Move it to (40,30)
...
def can_line_delete(gray, lines, threshold=0.3):
    # Get gray cv2 image with lines. Return cleaned image
    ...
    hp = Helper_Fuctions()
    # We use same logic with merging to find 4 points of bounding box check helper function
    boxes = hp.get_line_bounding_box(lines, angle_tolerance=5.0, distance_tolerance=5.0)

    for cnt in boxes:
        rect = cv2.minAreaRect(cnt)
        box = cv2.boxPoints(rect)
        box = np.int0(box)
        # Check if line is real or not
        # TUNE INTENSITY THRESHOLD
        # IT CAN TAKE VALUES IN 0-255
        threshold = threshold * 255
        if hp.filter_line(gray, box, threshold):
            cv2.fillPoly(gray, pts=[box], color=(255,255,255))
...
for image in images_new:
    ...
    big_lines_cleaned_gray = can_line_delete(gray.copy(), big_lines.copy(), BIG_LINE_DELETE_INTENSITY_THRESH)
    big_lines_cleaned_color = cv2.cvtColor(big_lines_cleaned_gray.copy(), cv2.COLOR_GRAY2BGR)

    ## Write results ####################################################################################
    ## Write results to the disk ########################################################################

    cv2.imwrite('./big_lines_deleted/big_deleted_'+image, big_lines_cleaned_gray)
    ...
    if len(small_lines.shape) < 1:
        cv2.imwrite('./all_lines_deleted/all_deleted_'+image, big_lines_cleaned_gray)
        continue
    ...
    ## Line Delete ######################################################################################
    ## Delete small lines from image ####################################################################

    small_lines_cleaned_gray = line_delete(big_lines_cleaned_gray.copy(), small_lines.copy(), SMALL_LINE_DELETE_INTENSITY_THRESH)
    ...
    #line_image = draw_lines(big_lines_cleaned_color.copy(), small_lines.copy(), threshold=SMALL_LINE_DELETE_INTENSITY_THRESH)

    ## Write results ####################################################################################
    ## Write results to the disk ########################################################################

    #blended = cv2.addWeighted(main_image.copy(), 0.4, line_image.copy(), 0.9, 0)
    cv2.imwrite('./all_lines_deleted/all_deleted_'+image, small_lines_cleaned_gray)