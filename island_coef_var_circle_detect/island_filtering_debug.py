...
def show_im(image, name='empty'):
    ...
    cv2.moveWindow(name, 300,300)  # Move it to (300,300)
    cv2.imshow(name, image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

def draw_coef_var(image, results):
    # Get results dictionary print coef_var values on outer islands
    for island_name in results.keys():
        c_mass_x, c_mass_y = results[island_name]['c_mass']
        coef_var = results[island_name]['coef_var']
        cv2.putText(image, "{:.1f}".format(coef_var), (int(c_mass_x), int(c_mass_y - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0,0,0), 1)
...
if __name__ == "__main__":
    ...
    for image_name in image_names:
        ...
        # Image Blending to see the differance
        image = cv2.addWeighted(image_undilated, 0.5, image_dilated, 0.5, 0)
        draw_coef_var(image, results)

        show_im(image, 'results')

        cv2.imwrite("./islands/island_" + image_name, image)
